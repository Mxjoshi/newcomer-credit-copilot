...
import { POLICY_RULES, checkPolicy } from "./policy";
import {
...
function severityOf(ruleId: string): "hard_fail" | "refer" {
  const rule = POLICY_RULES.find((r) => r.rule_id === ruleId);
...
export function combine(
  ...
  if (failed.some((p) => severityOf(p.rule_id) === "hard_fail")) return "decline";
  ...
  return failed.some((p) => severityOf(p.rule_id) === "refer") ? "refer" : "approve";
}

const fmt = (n: number) => `AED ${Math.round(n).toLocaleString("en-US")}`;

function ruleCounterfactual(ruleId: string, a: Applicant, app: Application): string | null {
  switch (ruleId) {
    case "rule-1":
      return `a monthly salary of ${fmt(PRODUCT_MIN_SALARY[app.product])} would meet the product minimum (rule 1)`;
    case "rule-2": {
      const headroom = DBR_CAP * a.monthly_salary_aed - a.existing_monthly_obligations_aed;
      if (headroom <= 0) {
        return "existing obligations alone exceed the 50 percent debt burden cap at this salary; no loan amount fits (rule 2)";
      }
      const maxAmount =
        (headroom * app.term_months) / (1 + (FLAT_ANNUAL_RATE * app.term_months) / 12);
      return `reducing the requested amount to about ${fmt(maxAmount)} would bring the debt burden ratio within the 50 percent cap (rule 2)`;
    }
    case "rule-3": {
      const maxTerm = Math.floor((MAX_AGE_AT_MATURITY - a.age_years) * 12);
      return maxTerm > 0
        ? `a term of ${maxTerm} months or less would keep age at maturity within ${MAX_AGE_AT_MATURITY} (rule 3)`
        : `age already exceeds the maturity limit for any loan term (rule 3)`;
    }
    case "rule-4": {
      const more = MIN_TENURE_MONTHS - a.job_tenure_months;
      return `reaching ${MIN_TENURE_MONTHS} months of tenure (${more} more) would clear the minimum tenure rule (rule 4)`;
    }
    case "rule-5":
    ...
    case "rule-6":
      return `reducing the requested amount to ${fmt(AMOUNT_SALARY_MULTIPLE * a.monthly_salary_aed)} (20 times salary) would clear the amount cap (rule 6)`;
    default:
      return null;
  }
}
...
export function counterfactuals(
  ...
  for (const p of policyResults) {
    if (!p.passed) {
      const line = ruleCounterfactual(p.rule_id, a, app);
...
export function assess(a: Applicant, app: Application): Decision {
  const score_result = score(a, app);
  const policy_results = checkPolicy(a, app);
  const recommendation = combine(score_result, policy_results);
  return {
    recommendation,
    risk_band: score_result.risk_band,
    explanation: "",
    reasons: [],
    counterfactuals: counterfactuals(a, app, score_result, policy_results, recommendation),
    score_result,
    policy_results,
    officer_action: "none",
  };
