...
function findingFor(rule: PolicyRule, a: Applicant, app: Application, passed: boolean): string {
  ...
  switch (rule.rule_id) {
    ...
    case "rule-2": {
      ...
      const pct = Number.isFinite(dbr) ? `${(dbr * 100).toFixed(1)} percent` : "not computable on zero salary";
      return passed ? `debt burden ratio ${pct}, within the 50 percent cap` : `debt burden ratio ${pct}, over the 50 percent cap`;
    }
    case "rule-3": {
    ...
    case "rule-5":
      return passed
        ? `visa has ${a.visa_months_remaining} months remaining, term is ${app.term_months}`
