...
| The scorecard is pack-driven too | Factor count, names, point tiers, and rationales live in the pack's scorecard block; uae v1.0 enables the five M1 factors at 0-20 points each. ScoreResult contains one ScoreFactor per enabled factor (a 4-factor pack renders 4 rows). Factor semantics (how salary headroom or settledness is computed) stay in code keyed by factor_id, the same boundary as the rules. The M5 guardrail moved into the pack with the values: changing a tier requires a written rationale next to it. |
| Decision gains validation_outcome (optional) | Set by the explanation layer (llm-integration.md logging): passed, passed_on_retry, or fell_back_to_template. Records how the explanation was produced, feeding the Phase 4 hallucination eval directly. Unset until the LLM step has run. |
